  ...
  return [
    {
      ...
      content: [
        ...
        "如果用户只说今天、今日、明天、后天、某天或几天后，没有明确几点/上午/下午/晚上，dueAt 默认使用该日 23:59:00+08:00；不要默认成 09:00。",
        "时间线索只能绑定到原文明确修饰的事项；不要把明天、今晚等时间词套到相邻任务或其他未明确带时间的事项。",
        "推荐标准按这个顺序判断：可开始性 > 清晰度 > 时间线索 > 重要性；在此基础上用四象限和项目管理方法辅助排序，不要新增四象限字段。",
        ...
        "status 只能是 organized 或 empty；不要输出 success、ok、completed、done 或其他状态词。",
        ...
        "items 每项只输出必要字段：id, title, sourceUnitIds, mentions, type, priority, assignTo, reason, nextStep, focusSteps, tags, isBigEvent；如识别到时间，再加入 timeHint、dueAt 或 remindDaysBefore。",
        "不要输出空字符串、空数组或 null 字段；没有信息的字段直接省略。",
        "recommendedNow 包含 itemId, title, reason, nextStep，需要指向 items 中的一项。",
        "meta 需要包含 modelBehavior 和 safetyLevel；modelBehavior 固定为 ai，safetyLevel 固定为 normal。",
